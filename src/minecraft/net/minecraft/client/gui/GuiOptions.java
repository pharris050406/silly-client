...
{																														
    private static final GameSettings.Options[] misc_options = new GameSettings.Options[] {GameSettings.Options.FOV};
    ...
    public void initGui()
    {
        ...
        GameSettings.Options[] optionsList = misc_options;
        int optionsLength = optionsList.length;

        for (int i = 0; i < optionsLength; ++i)
        {
            GameSettings.Options option = optionsList[i];
            if (option.getEnumFloat())
            {
                this.buttonList.add(new GuiOptionSlider(option.returnEnumOrdinal(), this.width / 2 - 155 + var1 % 2 * 160, this.height / 6 - 12 + 24 * (var1 >> 1), option));
            }
            else
            {
                GuiOptionButton var6 = new GuiOptionButton(option.returnEnumOrdinal(), this.width / 2 - 155 + var1 % 2 * 160, this.height / 6 - 12 + 24 * (var1 >> 1), option, this.game_settings_1.getKeyBinding(option));
                this.buttonList.add(var6);
            }
            	++var1;
        }
        
        // TODO adds the silly button with id 201
    	this.buttonList.add( new GuiButton(201, this.width / 2 - 155 + var1 % 2 * 160, this.height / 6 - 12 + 24 * (var1 >> 1), 150, 20, "silly settings"));
    	var1++;
    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        if (button.enabled)
        {
            ...
            // TODO silly button on click do thing
            if(button.id == 201) {
            	this.mc.displayGuiScreen(new GuiSillyOptions(this,this.game_settings_1));
            }
        }
    }
